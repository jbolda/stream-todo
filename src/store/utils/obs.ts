/**
 * This is a soft fork from https://github.com/obs-websocket-community-projects/obs-websocket-js/blob/master/src/types.ts
 * This file is autogenerated by scripts/build-types.ts
 * To update this with latest changes do npm run generate:obs-types
 */
import { call, Operation } from "starfx";
...
export function* authenticationHashing(
  ...
  const encoded = yield* call(() => digestMessage(msg + salt));
...
export type RequestMessage<T = keyof OBSRequestTypes> =
...
export type RequestBatchRequest<T = keyof OBSRequestTypes> =
  T extends keyof OBSRequestTypes
    ? OBSRequestTypes[T] extends never
      ? {
          requestType: T;
          requestId?: string;
        }
...
export type RequestBatchMessage = Merge<
  RequestBatchOptions,
  {
    requestId: string;
...
export type ResponseMessage<T = keyof OBSResponseTypes> =
